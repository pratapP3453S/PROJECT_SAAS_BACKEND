...
import { EmailJobData, JobResult } from '../../domain/interfaces/job.interface';

/**
 * EmailProcessor — BullMQ consumer for the email queue.
 *
 * Layer: infrastructure/processors — the concrete worker that picks up
 * SEND_EMAIL jobs from Redis and executes delivery. Errors are re-thrown so
 * Bull can apply the configured retry / back-off policy.
 *
 ...
 *
 * Producer: EmailQueue → src/modules/jobs/application/queues/email.queue.ts
 */
