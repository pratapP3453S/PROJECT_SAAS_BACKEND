 ...
 * is applied for this route in main.ts.
 *
 * URL versioning: this controller is intentionally NOT URI-versioned (no
 * `version` on @Controller). Signed URLs are bearer credentials issued for a
 * specific path; baking a version segment in would invalidate every
 * outstanding URL on a version bump. The /upload/presigned-url issuer in v1
 * is responsible for pointing at a stable /upload/local/direct path.
 *
...
export class LocalDirectUploadController {
  ...
  async upload(
    ...
    return {
      ...
      fileKey: key,
      size: stored.size,
      // Echo back what the client should now POST to /v1/upload/presigned-url/complete.
      next: { endpoint: '/v1/upload/presigned-url/complete', body: { fileKey: key } },
    };
  }
