...
import { createUploadMiddleware } from '../../../infrastructure/multer/multer.factory';
import {
  CommitFileDto,
 ...
 *
 * Per-category overrides (video gets 500 MB, archive gets 100 MB, etc.) are
 * still enforced by FileValidatorService against the registry AFTER Multer
...
export class UploadController {
  ...
  @UseInterceptors(
    FileInterceptor(
      'file',
      // Multer's file size limit here is the GLOBAL ceiling (MAX_FILE_SIZE_MB).
      // Per-category limits (video=500MB, archive=100MB, etc.) are enforced
      // afterward by FileValidatorService.validate(). Setting the Multer cap
      // to the biggest possible category would defeat the point — instead we
      // let through anything within the global maximum and let the validator
      // reject per-category overruns with a clean 415.
  ...
  @ApiParam({
    ...
    example: 'avatar',
    description:
      'Upload category. Built-in: avatar, image, video, audio, document, spreadsheet, ' +
      'presentation, archive, aadhar, identity, passport. Reserved tokens (commit, ' +
      'remove, presigned-url, download-url) are claimed by the literal routes above ' +
