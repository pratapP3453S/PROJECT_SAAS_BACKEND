...
export function setupSwagger(app: INestApplication): void {
  ...
  const config = new DocumentBuilder()
    .setTitle(appName)
    .setDescription(
      `## ${appName}\n\nEnterprise-level NestJS API with full DDD layer separation ` +
        `(domain / infrastructure / application / api) and URI-versioned routes.\n\n` +
        `### Authentication\nUse the **Authorize** button to set your Bearer token.\n\n` +
        `### Response Format\nAll responses follow a standardized envelope with \`success\`, ` +
        `\`statusCode\`, \`message\`, \`data\`, plus diagnostic \`request\` / \`timing\` / ` +
        `\`server\` blocks.`,
    )
